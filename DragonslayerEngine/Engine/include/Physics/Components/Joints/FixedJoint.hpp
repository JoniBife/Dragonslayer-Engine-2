...
ENGINE_DSTRUCT(FixedJoint, .numComponents = 1024, .hasMetadata = true, .isDisplayable = false) final  {

    GENERATE_DSTRUCT_BODY(FixedJoint)
    ...
    FixedJoint(
        ...
        const PxTransform& localFrameA = PxTransform(PxIdentity),
        const PxTransform& localFrameB = PxTransform(PxIdentity)
    ) : FixedJoint(bodyA.body, bodyB.body, localFrameA, localFrameB) {}
