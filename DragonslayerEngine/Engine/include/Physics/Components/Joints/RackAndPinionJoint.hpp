...
ENGINE_DSTRUCT(RackAndPinionJoint, .numComponents = 1024, .hasMetadata = true, .isDisplayable = false) final {

    GENERATE_DSTRUCT_BODY(RackAndPinionJoint)

    PxRackAndPinionJoint* joint = nullptr;
    ...
    RackAndPinionJoint(
        ...
        const PxTransform& localFrameA = PxTransform(PxIdentity),
        const PxTransform& localFrameB = PxTransform(PxIdentity)
    ) : RackAndPinionJoint(bodyA.body, bodyB.body, localFrameA, localFrameB) {}
...
}; END_DSTRUCT(RackAndPinionJoint)
