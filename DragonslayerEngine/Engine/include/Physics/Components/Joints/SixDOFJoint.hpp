...
ENGINE_DSTRUCT(SixDOFJoint, .numComponents = 1024, .hasMetadata = true, .isDisplayable = false) final {

    GENERATE_DSTRUCT_BODY(SixDOFJoint)
    ...
    SixDOFJoint(
        ...
        const PxTransform& localFrameA = PxTransform(PxIdentity),
        const PxTransform& localFrameB = PxTransform(PxIdentity)
    ) : SixDOFJoint(bodyA.body, bodyB.body, localFrameA, localFrameB) {}
